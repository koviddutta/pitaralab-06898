-- Fix Phase 1 Ingredient Architecture: Idempotent seed + unique index
-- This migration makes the ingredient seeding idempotent and adds count assertion

-- Step 1: Add unique index on ingredient name (if not exists)
CREATE UNIQUE INDEX IF NOT EXISTS idx_ingredients_name 
ON public.ingredients(name);

-- Step 2: Re-seed ingredients with UPSERT (idempotent)
-- This allows the migration to run multiple times safely
...
-- Step 3: Update sugar_split for fruits (idempotent)
UPDATE public.ingredients SET sugar_split = '{"glucose": 2, "fructose": 4.5, "sucrose": 8.3}'::jsonb WHERE name = 'Mango Pulp (Alphonso)';
UPDATE public.ingredients SET sugar_split = '{"glucose": 2.0, "fructose": 2.4, "sucrose": 3.28}'::jsonb WHERE name = 'Strawberry Puree';
UPDATE public.ingredients SET sugar_split = '{"glucose": 2.35, "fructose": 2.45, "sucrose": 4.7}'::jsonb WHERE name = 'Raspberry Puree';
UPDATE public.ingredients SET sugar_split = '{"glucose": 4.98, "fructose": 4.85, "sucrose": 13.01}'::jsonb WHERE name = 'Banana Puree';
UPDATE public.ingredients SET sugar_split = '{"glucose": 2.0, "fructose": 1.85, "sucrose": 7.97}'::jsonb WHERE name = 'Pineapple Puree';
UPDATE public.ingredients SET sugar_split = '{"glucose": 0.5, "fructose": 0.6, "sucrose": 1.42}'::jsonb WHERE name = 'Lemon Juice';
UPDATE public.ingredients SET sugar_split = '{"glucose": 2.25, "fructose": 2.55, "sucrose": 4.55}'::jsonb WHERE name = 'Orange Juice';
UPDATE public.ingredients SET sugar_split = '{"glucose": 3.6, "fructose": 4.2, "sucrose": 3.4}'::jsonb WHERE name = 'Passion Fruit Pulp';
UPDATE public.ingredients SET sugar_split = '{"glucose": 5.8, "fructose": 6.2, "sucrose": 4.53}'::jsonb WHERE name = 'Litchi Pulp';

-- Step 4: Seed count assertion
DO $$
DECLARE
  ingredient_count INT;
BEGIN
  SELECT COUNT(*) INTO ingredient_count FROM public.ingredients;
  IF ingredient_count < 11 THEN
    RAISE EXCEPTION 'Seed validation failed: Expected >= 11 ingredients, found only %. Migration incomplete.', ingredient_count;
  END IF;
  RAISE NOTICE '✅ Seed validation passed: % ingredients loaded', ingredient_count;
END $$;